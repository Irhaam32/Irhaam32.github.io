...
graph TD
    ...
    subgraph IngestionColumn ["🔵 INGESTION FLOW<br/>(When a Bot Creator uploads a document)"]
        %% Actors
        BC["Bot Creator<br/>(SaaS User)"]
        
        %% Backend Server Swimlane
        subgraph BackendServer1 ["Your Backend Server"]
            App1["Python Application<br/>(FastAPI)"]
            FAISS1["FAISS Library<br/>(In-Memory)"]
            
            subgraph LocalDisk1 ["Local Disk Storage"]
        ...
        %% Cloud APIs Swimlane
        subgraph CloudAPIs1 ["Cloud APIs"]
            Embed1["OpenAI Embedding API"]
        end
    end

    %% Column 2: QUERY FLOW
    subgraph QueryColumn ["🟢 QUERY FLOW<br/>(When an End-User chats with the bot)"]
        %% Actors
        EU["End-User<br/>(via Iframe)"]
        
        %% Backend Server Swimlane
        subgraph BackendServer2 ["Your Backend Server"]
            App2["Python Application<br/>(FastAPI)"]
            FAISS2["FAISS Library<br/>(In-Memory)"]
            
            subgraph LocalDisk2 ["Local Disk Storage"]
        ...
        %% Cloud APIs Swimlane
        subgraph CloudAPIs2 ["Cloud APIs"]
            Embed2["OpenAI Embedding API"]
            LLM1["LLM API<br/>(GPT-4o / Claude 3)"]
        end
    end

    %% Ingestion Flow Connections (Blue Arrows)
    ...
    %% Query Flow Connections (Green Arrows)
    ...
    class App1 appStyle
    class App2 appStyle
    class FAISS1 faissStyle
    class FAISS2 faissStyle
    class Folder1 folderStyle
    class Folder2 folderStyle2
    class Folder3 folderStyle2
    class Folder4 folderStyle3
    class Note1 noteStyle
    class Embed1 apiStyle
    class Embed2 apiStyle
    class LLM1 apiStyle